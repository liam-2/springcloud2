...
public class GatewayConfig {
    ...
    public RouteLocator customeRouteLocator(RouteLocatorBuilder routeLocatorBuilder) {
        ...
        routes.route("path_route_george", r -> r.path("/guonei").uri("http://news.baidu.com/guonei"));

        return routes.build();
    }
    @Bean
    public RouteLocator customeRouteLocator2(RouteLocatorBuilder routeLocatorBuilder) {
        RouteLocatorBuilder.Builder routes = routeLocatorBuilder.routes();
        routes.route("path_route_george", r -> r.path("/guoji").uri("http://news.baidu.com/guoji"));
